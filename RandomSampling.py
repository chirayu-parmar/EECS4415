...
def run(file_name, support_percentage, subset_percentage, main_folder_path, actual_folder_path):
    ...
    #------------- Random Sampling of data based on the subset size -------------
    # read file method timer
    method_start_time = time.time()
    buckets_data = random.sample(buckets_data, subset_size)
    # read file method end timer
    method_end_time = time.time()
    # Append the data with result string for final "_result file".
    result += f'Random Sampling function takes {method_end_time - method_start_time:.3f} seconds to execute.\n\n'
    # print the data on console.
    print(f'Random Sampling function takes {method_end_time - method_start_time:.3f} seconds to execute.')

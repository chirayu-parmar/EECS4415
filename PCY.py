...
def hash_function(item_a, item_b):
    # For retail
    # return ((item_b * item_b) + item_a) % 75000
    # For netflix
    return ((item_b * item_b) + item_a) % 5000000
...
def run(file_name, support_percentage, subset_percentage, main_folder_path, actual_folder_path):
    ...
    for bucket in buckets_data:
        ...
        for pair in list(itertools.combinations(bucket_items, 2)):
            hash_value = hash_function(int(pair[0]), int(pair[1]))
    ...
    result += f'Hash Table length: {len(hash_table)}.\n'
    # print the data on console. 
    ...
    # Store single item L1 frequent single item data count in "L1" text file.
    ...
    bit_vector = defaultdict(bool)
    for key, value in hash_table.items():
        if value >= support_threshold:
            bit_vector[key] = True
        else:
            bit_vector[key] = False
    method_end_time = time.time()
    
    del hash_table # remove hash_table from the memory.
    # Append the data with result string for final over "_result file".
    result += f'Bit vector function takes {method_end_time - method_start_time:.3f} seconds of time to execute.\n'
    print(f'Bit vector function takes {method_end_time - method_start_time:.3f} seconds of time to execute.')
    ...
    for pair in list(itertools.combinations(sorted(frequent_single_items.keys()), 2)):
        # hash_value = hash_function(pair[0], pair[1], hash_table_size)
        hash_value = hash_function(pair[0], pair[1])
        if bit_vector[hash_value]:
